...
def coerce_model_ids(models_value: Any) -> list[str]:
    if not models_value:
        return []
...
def pick_default_model(provider: Dict[str, Any], requirements: Dict[str, Any]) -> Optional[str]:
    explicit = requirements.get("model")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()

    model_ids = coerce_model_ids(provider.get("models"))
    if not model_ids:
        return None
...
def ensure_provider_model(
    ...
    if not (isinstance(chosen_model, str) and chosen_model.strip()):
        chosen_model = pick_default_model(selected_provider, requirements or {})
        if chosen_model:
...
def derive_system_prompt_from_requirements(requirements: Dict[str, Any]) -> Optional[str]:
...
async def safe_rate_limit_check(autoscaling_service, client_ip: Optional[str], user_id: Optional[str]):
    try:
        return await autoscaling_service.check_rate_limit(
        ...
        return True, FallbackLevel.NORMAL, {}


def build_local_routing_metadata(
    messages: List[Dict[str, Any]],
    context_provided: Optional[str],
    intent_hint: Optional[Intent],
    latency_target: Optional[LatencyTarget],
    model_id: str,
) -> tuple[Intent, str, int, str]:
    detected_intent = intent_hint or detect_intent(messages)
    ...
    return detected_intent, system_prompt, context_length, explanation
...
async def handle_local_llm_routing(
    ...
    try:
        ...
        (
            ...
            fallback_level,
            rate_limit_metadata,
        ) = await safe_rate_limit_check(
            autoscaling_service, params["client_ip"], params["user_id"]
        )
        ...
        (
            ...
            context_length,
            explanation,
        ) = build_local_routing_metadata(
            params["messages"],
            params["context_provided"],
            params["intent"],
...
async def handle_provider_selection_and_fallback(
    ...
    selected_provider, model_error = ensure_provider_model(
        selected_provider, requirements or {}, request_id
    )
    if model_error:
        return model_error
    ...
    system_prompt = derive_system_prompt_from_requirements(requirements or {})
